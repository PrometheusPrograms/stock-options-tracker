-- Migration: Change ARORC from percentage back to decimal and update column type to NUMERIC(10,4)
-- ARORC is currently stored as a percentage (50.0 = 50%), but should be stored as a decimal (0.5 = 50%)
-- This migration converts existing values from percentage to decimal (divide by 100)
-- and changes the column type to NUMERIC(10,4) to store 4 decimal places
...
-- Create new trades table with ARORC as NUMERIC(10,4)
...
-- Copy data from old table to new table, converting ARORC from percentage to decimal
-- If ARORC is NULL, keep it as NULL
-- If ARORC is > 1, assume it's percentage and divide by 100
-- If ARORC is <= 1, assume it's already decimal (from new calculations)
...
SELECT
    ...
    CASE 
        ...
        WHEN ARORC > 1.0 THEN ROUND(ARORC / 100.0, 4)
        ELSE ROUND(ARORC, 4)
    END AS ARORC,
    trade_type_id
FROM trades;
